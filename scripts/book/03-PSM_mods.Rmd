...
combine_fun <- function(trait, outcome, mod, prob){
  ...
  if(mod != "none"){
    ...
    save(pred.fx, pred.rx, file = sprintf("%s/results/psm/matched/89-CI/predicted/matched_pred_%s_%s_%s.RData", wd, trait, outcome, mod))
    rm(list = c("pred.fx", "pred.rx"))
  }
# save variance covariance matrix, fixed and random summaries, 
# and posterior draws to folders
  save(vc, file = sprintf("%s/results/psm/matched/89-CI/vc/matched_%s_%s_%s.RData", wd, trait, outcome, mod))
  save(fx, rx, file = sprintf("%s/results/psm/matched/89-CI/summary/matched_%s_%s_%s.RData", wd, trait, outcome, mod))
  save(fx.draws, tau.draws, file = sprintf("%s/results/psm/matched/89-CI/draws/matched_%s_%s_%s.RData", wd, trait, outcome, mod))
...
done <- tibble(file = list.files(sprintf("%s/results/psm/matched/89-CI/summary", wd))) %>%
...
load_fun <- function(file, ob){
  load(sprintf("%s/results/psm/matched/89-CI/summary/%s", wd, file))
  get(ls()[ls() == ob])
}

# load in fixed and random summaries
fx_res <- tibble(file = list.files(sprintf("%s/results/psm/matched/89-CI/summary", wd))) %>%
...
plot_fun <- function(df, outcome, mod){
  ...
  ggsave(p, file = sprintf("%s/results/psm/matched/89-CI/plots/study specific forest/%s/matched_%s.pdf", wd, mod, outcome), width = 7, height = 4)
  save(p, file = sprintf("%s/results/psm/matched/89-CI/plots/study specific forest/rdata/matched_%s_%s.RData", wd, outcome, mod))
  rm(p)
  gc()
  return(T)
}
...
ggsave(file = sprintf("%s/results/psm/matched/89-CI/plots/overall forest/%s_fixed.png", wd, mod), width = 7, height = 6)
save(p, file = sprintf("%s/results/psm/matched/89-CI/plots/overall forest/rdata/%s_fixed.RData", wd, mod))
rm(p)
gc()
return(T)
}
...
psm_tab_fun <- function(d, mod){
 ...
 rs <- tab %>% group_by(Outcome) %>% tally() %>% 
  ...
  tab %>%
    ...
    save_kable(., file = sprintf("%s/results/psm/matched/89-CI/tables/study specific/%s.html", wd, mod))
}
...
psm_fxd_tab_fun <- function(d, mod){
  ...
  d %>% 
    ...
    add_header_above(t_cols) %>%
    footnote("<em>OR</em> = Odds Ratio; <em>UI</em> = 89% Bayesian Uncertainty Interval. Bold indicates model terms whose 89% UI of log odds did not overlap with 0.", escape = F) %>%
    save_kable(., file = sprintf("%s/results/psm/matched/89-CI/tables/fixed/%s.html", wd, mod))
}
...
fx_res %>%
  ...
  filter(!is.na(Term)) %>%
    ...
    footnote("<em>OR</em> = Odds Ratio; <em>UI</em> = 89% Bayesian Uncertainty Interval. Bold indicates model terms whose 89% UI of log odds did not overlap with 0. Binary variables were dummy coded such that the label to the right of the 'v' indicates the reference group.", escape = F) %>% save_kable(., file = sprintf("%s/results/psm/matched/89-CI/tables/fixed/all_moderators.html", wd))
```
...
```{r vc mats}
load_fun <- function(file, ob){
  load(sprintf("%s/results/psm/matched/89-CI/vc/%s", wd, file))
  get(ls()[ls() == ob])
}
...
vc_res <- tibble(file = list.files(sprintf("%s/results/psm/matched/89-CI/vc", wd))) %>%
  separate(file, c("scrap", "Trait", "Outcome", "Moderator"), sep = "_", remove = F) %>%
  filter(Trait %in% traits$short_name) %>%
  mutate(Moderator = str_remove(Moderator, ".RData"),
...
vc_var %>%
  ...
  save_kable(., file = sprintf("%s/results/psm/matched/89-CI/tables/level 2/p_value_none.html", wd))
...
vc_var %>%
  ...
  kable_styling(full_width = F) %>%
  add_header_above(hcols) %>%
  save_kable(., file = sprintf("%s/results/psm/matched/89-CI/tables/level 2/all_mods.html", wd))
...
vc_tab_fun <- function(d, mod){
  ...
  kable_styling(full_width = F) %>%
  add_header_above(hcols) %>%
  save_kable(., file = sprintf("%s/results/psm/matched/89-CI/tables/level 2/%s.html", wd, mod))
}
...
load_fun <- function(file, ob){
  load(sprintf("%s/results/psm/matched/89-CI/predicted/%s", wd, file))
  get(ls()[ls() == ob])
}

fx_pred <- tibble(file = list.files(sprintf("%s/results/psm/matched/89-CI/predicted", wd))) %>%
  ...
  df %>%
    ...
    ggplot(aes(x = p_value
      ...
      theme(legend.position = "bottom"
            ...
            , strip.text = element_text(face = "bold", color = "white")
            , axis.text = element_text(color = "black"))
  ggsave(file = sprintf("%s/results/psm/matched/89-CI/plots/simple effects/%s/matched_%s.pdf", wd, mod, outcome), width = 6, height = 6)
}
  ...
  p<- df %>%
    ...
    ggplot(aes(x = p_value
      ...
      theme(legend.position = "bottom"
            ...
            , axis.text = element_text(color = "black"))
  ggsave(p, file = sprintf("%s/results/psm/matched/89-CI/plots/simple effects/reliable/matched_%s.pdf", wd, mod), width = ncols*2+2, height = nrows*2)
  save(p, file = sprintf("%s/results/psm/matched/89-CI/plots/simple effects/reliable/matched_%s.RData", wd, mod))
}
...
load(sprintf("%s/results/psm/matched/89-CI/plots/simple effects/reliable/matched_%s.RData", res_path, "age"))
p
```
...
load(sprintf("%s/results/psm/matched/89-CI/plots/simple effects/reliable/matched_%s.RData", res_path, "gender"))
p
```
