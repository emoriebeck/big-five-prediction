...
fx_pred_fun <- function(fits, m, pr){
  ...
  pred.fx <- bind_cols(
    ...
    setNames(c(colnames(pred.fx), "Estimate", "Est.Error", "lower", "upper")) %>%
    mutate_at(vars(Estimate, lower, upper), lst(OR = exp))
}
...
rx_pred_fun <- function(fits, m, pr){
  ...
  pred.rx <- bind_cols(
    ...
    setNames(c(colnames(pred.rx), "Estimate", "Est.Error", "lower", "upper")) %>%
    mutate_at(vars(Estimate, lower, upper), lst(OR = exp))
  return(pred.rx)
}
```
...
combine_fun <- function(trait, outcome, mod, prob){
  pr <- if(prob == "95%") c(.025, .975) else c(0.055, 0.945)
  ...
  # extract key parameters
  # fixed effects
  fx <- fixef(fits, probs = pr) %>% data.frame %>%
    rownames_to_column("names") %>%
    setNames(c("names", "Estimate", "Est.Error", "lower", "upper")) %>%
    mutate_at(vars(-names), lst(OR = exp)) %>%
    as_tibble()
  # random effects
  rx <- coef(fits, probs = pr)[[1]] %>% array_tree(3) %>%
    tibble(names = names(.), data = .) %>%
    mutate(data = map(data, ~(.) %>% data.frame %>%
                        rownames_to_column("study"))) %>%
    unnest(data) %>%
    setNames(c("names", "study", "Estimate", "Est.Error", "lower", "upper")) %>%
    mutate_at(vars(-names, -study), lst(OR = exp)) %>%
    as_tibble()
  
  # variance covariance matrix
  vc <- VarCorr(fits, probs = pr)$study
  vc <- llply(vc, function(x) {colnames(x) <- c("Estimate", "Est.Error", "lower", "upper"); x})
  ...
  if(mod != "none"){
    pred.fx <- fx_pred_fun(fits, m, pr)
    pred.rx <- rx_pred_fun(fits, m, pr)
    save(pred.fx, pred.rx, file = sprintf("%s/results/psm/matched/95-CI/predicted/matched_pred_%s_%s_%s.RData", wd, trait, outcome, mod))
    rm(list = c("pred.fx", "pred.rx"))
  }
# save variance covariance matrix, fixed and random summaries, 
# and posterior draws to folders
  save(vc, file = sprintf("%s/results/psm/matched/95-CI/vc/matched_%s_%s_%s.RData", wd, trait, outcome, mod))
  save(fx, rx, file = sprintf("%s/results/psm/matched/95-CI/summary/matched_%s_%s_%s.RData", wd, trait, outcome, mod))
  save(fx.draws, tau.draws, file = sprintf("%s/results/psm/matched/95-CI/draws/matched_%s_%s_%s.RData", wd, trait, outcome, mod))
...
done <- tibble(file = list.files(sprintf("%s/results/psm/matched/95-CI/summary", wd))) %>%
...
res <- tibble(file = files) %>%
  ...
  # filter(is.na(Done)) %>%
  mutate(prob = "89%", 
         c = future_pmap(list(Trait, Outcome, Moderator, prob)
                         , safely(combine_fun)
                         , .progress = T))
closeAllConnections()
```
...
load_fun <- function(file, ob){
  load(sprintf("%s/results/psm/matched/95-CI/summary/%s", wd, file))
  get(ls()[ls() == ob])
}

# load in fixed and random summaries
fx_res <- tibble(file = list.files(sprintf("%s/results/psm/matched/95-CI/summary", wd))) %>%
  separate(file, c("scrap", "Trait", "Outcome", "Moderator"), sep = "_", remove = F) %>%
  filter(Trait %in% traits$short_name) %>%
  mutate(Moderator = str_remove(Moderator, ".RData"),
...
# %s significant 
cat(round(nrow(fx_res %>% filter(names == "p_value" & Moderator == "none") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the tested unmoderated, matched personality-outcome associations were significant.", sep ="")
...
cat(round(nrow(fx_res %>% filter(names == "p_value:age" & Moderator == "age") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by age.", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:gender1" & Moderator == "gender") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by gender", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:race1" & Moderator == "race") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by race (black v white)", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:race2" & Moderator == "race") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by race (other v white)", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:parEdu1" & Moderator == "parEdu") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by parental education (SES; college v HS or lower)", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:parEdu2" & Moderator == "parEdu") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by parental education (SES; beyond college v HS or lower)", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:parOccPrstg" & Moderator == "parOccPrstg") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by parental occupational prestige (SES)", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:grsWages" & Moderator == "grsWages") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by gross wages (SES)", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:reliability" & Moderator == "reliability") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by personality scale reliability", sep ="")

cat(round(nrow(fx_res %>% filter(names == "p_value:predInt" & Moderator == "predInt") %>% filter(sign(lower) == sign(upper)))/70*100,2), "% of the matched personality-outcome associations were significantly moderated by the interval between personality and outcome measures", sep ="")
...
plot_fun <- function(df, outcome, mod){
  ...
  lim <- if(mod == "none"){c(.78, 1.22)} else{c(1-d-(d/2.5), 1+d+(d/2.5))}
  ...
  p <- df %>%
    mutate(lower_OR = ifelse(lower_OR < lim[1], lim[1], lower_OR),
           upper_OR = ifelse(upper_OR > lim[2], lim[2], upper_OR),
    ...
    ggplot(aes(x = study, y = Estimate_OR)) +
      ...
      geom_errorbar(aes(ymin = lower_OR, ymax = upper_OR, linetype = names)
                    , width = 0
  ...
  ggsave(p, file = sprintf("%s/results/psm/matched/95-CI/plots/study specific forest/%s/matched_%s.pdf", wd, mod, outcome), width = 7, height = 4)
  save(p, file = sprintf("%s/results/psm/matched/95-CI/plots/study specific forest/rdata/matched_%s_%s.RData", wd, outcome, mod))
  rm(p)
  gc()
  return(T)
}
...
ggsave(file = sprintf("%s/results/psm/matched/95-CI/plots/overall forest/%s_fixed.png", wd, mod), width = 7, height = 6)
save(p, file = sprintf("%s/results/psm/matched/95-CI/plots/overall forest/rdata/%s_fixed.RData", wd, mod))
rm(p)
gc()
return(T)
}
...
psm_tab_fun <- function(d, mod){
  ...
  tab <- d %>%
    ...
    mutate_at(vars(Estimate_OR:upper_OR), ~sprintf("%.2f", .)) %>%
    mutate(est = sprintf("%s [%s, %s]", Estimate_OR, lower_OR, upper_OR),
 ...
 rs <- tab %>% group_by(Outcome) %>% tally() %>% 
  ...
  tab %>%
    ...
    save_kable(., file = sprintf("%s/results/psm/matched/95-CI/tables/study specific/%s.html", wd, mod))
}
...
psm_fxd_tab_fun <- function(d, mod){
  ...
  d %>% 
    mutate_at(vars(Estimate_OR:upper_OR), ~sprintf("%.2f", .)) %>%
    mutate(est = sprintf("%s<br>[%s, %s]", Estimate_OR, lower_OR, upper_OR),
           est = ifelse(sig == "sig", sprintf("<strong>%s</strong>", est), est)) %>%
    ...
    add_header_above(t_cols) %>%
    footnote("<em>OR</em> = Odds Ratio; <em>UI</em> = 89% Bayesian Uncertainty Interval. Bold indicates model terms whose 89% UI of log odds did not overlap with 0.", escape = F) %>%
    save_kable(., file = sprintf("%s/results/psm/matched/95-CI/tables/fixed/%s.html", wd, mod))
}
...
fx_res %>%
  ...
  mutate(sig = ifelse(sign(lower) == sign(upper), "sig", "ns")) %>%
  select(-(Estimate:upper)) %>%mutate_at(vars(Estimate_OR:upper_OR), ~sprintf("%.2f", .)) %>%
    mutate(est = sprintf("%s<br>[%s, %s]", Estimate_OR, lower_OR, upper_OR),
           est = ifelse(sig == "sig", sprintf("<strong>%s</strong>", est), est)) %>%
    select(Trait, Outcome, Term = names, est) %>%
    # spread it wide and order Outcomes, columns, and studies
  ...
  filter(!is.na(Term)) %>%
    ...
    kable_styling(full_width = F) %>%
    kableExtra::group_rows("Extraversion", rs$start[1], rs$end[1]) %>%
    kableExtra::group_rows("Agreeableness", rs$start[2], rs$end[2]) %>%
    kableExtra::group_rows("Conscientiousness", rs$start[3], rs$end[3]) %>%
    kableExtra::group_rows("Neuroticism",rs$start[4], rs$end[4]) %>%
    kableExtra::group_rows("Openness to Experience", rs$start[5], rs$end[5]) %>%
    # collapse_rows(1, valign = "top") %>%
    add_header_above(r_cols) %>%
    footnote("<em>OR</em> = Odds Ratio; <em>UI</em> = 89% Bayesian Uncertainty Interval. Bold indicates model terms whose 89% UI of log odds did not overlap with 0. Binary variables were dummy coded such that the label to the right of the 'v' indicates the reference group.", escape = F) %>% save_kable(., file = sprintf("%s/results/psm/matched/95-CI/tables/fixed/all_moderators.html", wd))
```
...
fx_mod <- fx_res %>%
  filter(grepl("p_value:", names) & sign(lower) == sign(upper)) 
fx_mod
```
...
```{r vc mats}
load_fun <- function(file, ob){
  load(sprintf("%s/results/psm/matched/95-CI/vc/%s", wd, file))
  get(ls()[ls() == ob])
}
...
vc_res <- tibble(file = list.files(sprintf("%s/results/psm/matched/95-CI/vc", wd))) %>%
  separate(file, c("scrap", "Trait", "Outcome", "Moderator"), sep = "_", remove = F) %>%
  filter(Trait %in% traits$short_name) %>%
  mutate(Moderator = str_remove(Moderator, ".RData"),
...
vc_var <- vc_res %>% 
  ...
  filter(complete.cases(.))
  # mutate(sig = ifelse(sign(lower) == sign(upper), "sig", "ns")) %>%
  # mutate_at(vars(Estimate, lower, upper), ~(.)^2)
...
vc_var %>%
  ...
  save_kable(., file = sprintf("%s/results/psm/matched/95-CI/tables/level 2/p_value_none.html", wd))

hcols <- rep(1,6)
names(hcols) <- c(" ", traits$short_name)
vc_var %>%
  mutate_at(vars(Estimate, lower, upper), ~sprintf("%.3f", .)) %>%
  mutate_at(vars(Estimate, lower, upper), ~str_remove(., "^0")) %>%
  mutate(est = sprintf("%s<br>[%s, %s]", Estimate, lower, upper),
  ...
  kable_styling(full_width = F) %>%
  add_header_above(hcols) %>%
  save_kable(., file = sprintf("%s/results/psm/matched/95-CI/tables/level 2/all_mods.html", wd))
...
vc_tab_fun <- function(d, mod){
  ...
  kable_styling(full_width = F) %>%
  add_header_above(hcols) %>%
  save_kable(., file = sprintf("%s/results/psm/matched/95-CI/tables/level 2/%s.html", wd, mod))
}
...
load_fun <- function(file, ob){
  load(sprintf("%s/results/psm/matched/95-CI/predicted/%s", wd, file))
  get(ls()[ls() == ob])
}

fx_pred <- tibble(file = list.files(sprintf("%s/results/psm/matched/95-CI/predicted", wd))) %>%
...
simp_eff_fun <- function(df, outcome, mod){ 
  ...
  df %>%
    mutate(Trait = factor(Trait, levels = traits$short_name, labels = traits$long_name),
           lower_OR = ifelse(lower_OR < mini, mini, lower_OR),
           upper_OR = ifelse(upper_OR > maxi, maxi, upper_OR)) %>%
    ggplot(aes(x = p_value
      ...
      theme(legend.position = "bottom"
            ...
            , strip.text = element_text(face = "bold", color = "white")
            , axis.text = element_text(color = "black"))
  ggsave(file = sprintf("%s/results/psm/matched/95-CI/plots/simple effects/%s/matched_%s.pdf", wd, mod, outcome), width = 6, height = 6)
}
...
fx_pred %>%
  ...
  nest() %>%
  ungroup() %>%
  mutate(data = map(data, ~(.) %>% unnest(fx) %>% mutate_at(vars(Estimate, lower, upper), lst(OR = exp))),
  ...
  p<- df %>%
    ...
    ggplot(aes(x = p_value
      ...
      theme(legend.position = "bottom"
            ...
            , axis.text = element_text(color = "black"))
  ggsave(p, file = sprintf("%s/results/psm/matched/95-CI/plots/simple effects/reliable/matched_%s.pdf", wd, mod), width = ncols*2+2, height = nrows*2)
  save(p, file = sprintf("%s/results/psm/matched/95-CI/plots/simple effects/reliable/matched_%s.RData", wd, mod))
}
...
```{r, fig.cap = "Figure S1. Simple effects of all personality-outcome associations that were reliably moderated by age. Each panel indicates a separate personality-outcome association in terms of the odds ratio associated with a particular personality score for different age groups. Solid lines represent within-study-centered average age (i.e. 0), while dashed and dotted lines represent individuals in each study 1 SD above or below the study-centered average age, respectively. Shaded regions represent the 89% credible interval around the fitted estimates. Personality characteristics are presented as Percentage of Maximum Possible (POMP), which ranges from 0-10 for each study, such that 10 represents the maximum possible score, while 0 represents the minimum possible score.", fig.width=10, fig.height=6}
load(sprintf("%s/results/psm/matched/95-CI/plots/simple effects/reliable/matched_%s.RData", res_path, "age"))
p
```

#### Supplementary Figure S2  
```{r, fig.cap = "Figure S2. Simple effects of all personality-outcome associations that were reliably moderated by gender (0 = male, 1 = female). Each panel indicates a separate personality-outcome association in terms of the odds ratio associated with a particular personality score for different gender groups. Dotted lines represent individuals who identified as male, while solid lines represent individuals who identified as female. Shaded regions represent the 89% credible interval around the fitted estimates. Personality characteristics are presented as Percentage of Maximum Possible (POMP), which ranges from 0-10 for each study, such that 10 represents the maximum possible score, while 0 represents the minimum possible score.", fig.width=10, fig.height=6}
load(sprintf("%s/results/psm/matched/95-CI/plots/simple effects/reliable/matched_%s.RData", res_path, "gender"))
p
```
